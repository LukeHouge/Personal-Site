...
export class Helicopter extends GrObject {
  ...
  constructor(params = {}) {
    ...
    let helicopterBodyGeom = new T.Geometry();
    let q = 0.25;
    let body = 1;
    ...
    const helifaces = [
      0, 1, 4, 1, 2, 4, 2, 3, 4, 3, 0, 4, 1, 0, 5, 2, 1, 5, 3, 2, 5, 0, 3, 5, 6,
      7, 10, 7, 8, 10, 8, 9, 10, 9, 6, 10,
    ];
    ...
    for (let i = 0; i < helifaces.length; i += 3) {
      helicopterBodyGeom.faces.push(
        new T.Face3(helifaces[i], helifaces[i + 1], helifaces[i + 2])
      );
    }
    ...
    for (let i = 0; i < rotorfaces.length; i += 3) {
      this.rotorGeom.faces.push(
        new T.Face3(rotorfaces[i], rotorfaces[i + 1], rotorfaces[i + 2])
      );
    }
