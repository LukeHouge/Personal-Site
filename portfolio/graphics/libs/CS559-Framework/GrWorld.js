...
export class GrWorld {
  ...
  constructor(params = {}) {
    ...
    this.scene.background = new T.Color(
      params.background ? params.background : "black"
    );
    ...
    if (this.groundplane) this.add(this.groundplane);
   ...
   */
  animate(callbacks = {}) {
    if (!this.animCount && callbacks.prefirst) {
      callbacks.prefirst(this);
    }
    ...
    if (!this.animCount && callbacks.first) {
      callbacks.first(this);
    }
    this.animCount += 1;
  }
