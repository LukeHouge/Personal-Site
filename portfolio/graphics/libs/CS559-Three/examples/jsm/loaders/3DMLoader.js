...
Rhino3dmLoader.prototype = Object.assign(Object.create(Loader.prototype), {
  ...
  _createMaterial: function (material) {
    ...
    var diffusecolor = new Color(
      _diffuseColor.r / 255.0,
      _diffuseColor.g / 255.0,
      _diffuseColor.b / 255.0
    );
      ...
      switch (obj.objectType) {
        ...
        default:
          if (attributes.hasOwnProperty("materialUUID")) {
            var rMaterial = materials.find(
              (m) => m.id === attributes.materialUUID
            );
          ...
          _object.visible = layer
            ? data.layers[attributes.layerIndex].visible
            : true;
    ...
    switch (obj.objectType) {
      ...
      case "Light":
        ...
        if (geometry.isDirectionalLight) {
          ...
          light.position.set(
            geometry.location[0] - height / 2,
            geometry.location[1],
            geometry.location[2] - width / 2
          );
          ...
          console.warn(
            "THREE.3DMLoader:  No conversion exists for linear lights."
          );
  ...
  _initLibrary: function () {
    if (!this.libraryPending) {
      ...
      this.libraryPending = Promise.all([jsContent, binaryContent]).then(
      ...
      if (this.workerPool.length < this.workerLimit) {
        ...
        worker.onmessage = function (e) {
          ...
          switch (message.type) {
            ...
            default:
              console.error(
                'THREE.Rhino3dmLoader: Unexpected message, "' +
                  message.type +
                  '"'
              );
          }
...
Rhino3dmLoader.Rhino3dmWorker = function () {
  ...
  function decodeObjects(rhino, buffer) {
    ...
    for (var i = 0; i < cnt; i++) {
      ...
      if (object !== undefined) {
        if (object.attributes.materialIndex >= 0) {
          var mId = doc
            .materials()
            .findIndex(object.attributes.materialIndex).id;
    ...
    for (var i = 0; i < doc.materials().count(); i++) {
      ...
      for (var j = 0; j < textureTypes.length; j++) {
        ...
        if (_texture) {
          ...
          if (image) {
            ...
            console.warn(
              `THREE.3DMLoader: Image for ${textureType} texture not embedded in file.`
            );
            texture.image = null;
          }
      ...
      if (_pbrMaterial.supported) {
        ...
        var pbMaterialProperties = extractProperties(
  ...
  function extractObjectData(object, doc) {
    ...
    switch (objectType) {
      ...
      default:
        console.warn(
          `THREE.3DMLoader: TODO: Implement ${objectType.constructor.name}`
        );
        break;
    }
    ...
    if (Array.isArray(geometry) === false || geometry.length > 0) {
      ...
      console.warn(
        "THREE.3DMLoader: Object has no mesh geometry. Consider opening this in Rhino, using a shaded display mode, and exporting again."
      );
    }
  }
  ...
  function curveToPoints(curve, pointLimit) {
    ...
    for (var j = 0; j < pointCount; j++) {
      ...
      if (denominator === 0) {
        ...
        var theta =
          (tan.x * prevTan.x + tan.y * prevTan.y + tan.z * prevTan.z) /
          denominator;
