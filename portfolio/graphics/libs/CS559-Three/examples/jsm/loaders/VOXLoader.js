...
class VOXLoader extends Loader {
  ...
  parse(buffer) {
    ...
    while (i < data.byteLength) {
      ...
      const chunkSize = data.getInt32(i, true);
      i += 4;
      data.getInt32(i, true);
      i += 4; // childChunks

      if (id === "SIZE") {
        const x = data.getInt32(i, true);
        i += 4;
        const y = data.getInt32(i, true);
        i += 4;
        const z = data.getInt32(i, true);
        i += 4;
        ...
        const numVoxels = data.getInt32(i, true);
        i += 4;
        ...
        for (let j = 0; j < 256; j++) {
          palette[j + 1] = data.getInt32(i, true);
          i += 4;
        }
