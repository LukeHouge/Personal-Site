...
BasisTextureLoader.prototype = Object.assign(Object.create(Loader.prototype), {
  ...
  detectSupport: function (renderer) {
    ...
    config.astcSupported = renderer.extensions.has(
      "WEBGL_compressed_texture_astc"
    );
    config.bptcSupported = renderer.extensions.has(
      "EXT_texture_compression_bptc"
    );
    config.etcSupported = renderer.extensions.has(
      "WEBGL_compressed_texture_etc1"
    );
    config.dxtSupported = renderer.extensions.has(
  ...
  load: function (url, onLoad, onProgress, onError) {
    ...
    loader.load(
      ...
      (buffer) => {
        ...
        this._createTexture(buffer, url).then(onLoad).catch(onError);
      },
      onProgress,
      onError
    );
    ...
    var texturePending = this._allocateWorker(taskCost)
      .then((_worker) => {
        ...
        return new Promise((resolve, reject) => {
          ...
          worker.postMessage({ type: "transcode", id: taskID, buffer }, [
            buffer,
          ]);
        });
      })
        ...
        switch (format) {
          ...
          case BasisTextureLoader.BASIS_FORMAT.cTFBC3:
            texture = new CompressedTexture(
              ...
              width,
              height,
              BasisTextureLoader.DXT_FORMAT_MAP[config.format],
              UnsignedByteType
            );
            break;
    ...
    switch (message.type) {
      ...
      case "transcode":
        transcoderPending.then(() => {
          try {
            var { width, height, hasAlpha, mipmaps, format } = transcode(
              message.buffer
            );
  ...
  function transcode(buffer) {
    ...
    for (var mip = 0; mip < levels; mip++) {
      ...
      var dst = new Uint8Array(
        basisFile.getImageTranscodedSizeInBytes(0, mip, config.format)
      );
