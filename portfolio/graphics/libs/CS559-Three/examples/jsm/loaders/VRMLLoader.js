...
var VRMLLoader = (function () {
  ...
  if (typeof chevrotain === "undefined") {
    ...
    throw Error(
      "THREE.VRMLLoader: External library chevrotain.min.js required."
    );
  }
    ...
    parse: function (data, path) {
      ...
      function createTokens() {
        ...
        var Identifier = createToken({
          name: "Identifier",
          pattern:
            /[^\x30-\x39\0-\x20\x22\x27\x23\x2b\x2c\x2d\x2e\x5b\x5d\x5c\x7b\x7d][^\0-\x20\x22\x27\x23\x2b\x2c\x2d\x2e\x5b\x5d\x5c\x7b\x7d]*/,
      ...
      function createVisitor(BaseVRMLVisitor) {
        ...
        VRMLToASTVisitor.prototype = Object.assign(
          Object.create(BaseVRMLVisitor.prototype),
          {
      ...
      function buildBackgroundNode(node) {
        ...
        if (groundColor) {
          if (groundColor.length > 0) {
            var groundGeometry = new SphereBufferGeometry(
              radius,
              32,
              16,
              0,
      ...
      function buildMaterialNode(node) {
        ...
        for (var i = 0, l = fields.length; i < l; i++) {
          ...
          switch (fieldName) {
            ...
            case "diffuseColor":
              materialData.diffuseColor = new Color(
                fieldValues[0],
                fieldValues[1],
                fieldValues[2]
              );
            ...
            case "emissiveColor":
              materialData.emissiveColor = new Color(
                fieldValues[0],
                fieldValues[1],
                fieldValues[2]
              );
        ...
        for (var i = 0, l = fields.length; i < l; i++) {
          ...
          switch (fieldName) {
            case "image":
              ...
              for (
                var j = 3, k = 0, jl = fieldValues.length;
                j < jl;
                j++, k++
              ) {
              ...
              texture = new DataTexture(
                ...
                width,
                height,
                useAlpha === true ? RGBAFormat : RGBFormat
              );
      ...
      function buildIndexedFaceSetNode(node) {
        ...
        if (color) {
          if (colorPerVertex === true) {
            if (colorIndex && colorIndex.length > 0) {
              ...
              var triangulatedColorIndex = triangulateFaceIndex(
                colorIndex,
                ccw
              );
              colorAttribute = computeAttributeFromIndexedData(
                triangulatedCoordIndex,
                triangulatedColorIndex,
                color,
                3
              );
            } else {
              ...
              colorAttribute = toNonIndexedAttribute(
                triangulatedCoordIndex,
            ...
            if (colorIndex && colorIndex.length > 0) {
              ...
              var triangulatedFaceColors = triangulateFaceData(
                flattenFaceColors,
                coordIndex
              );
              colorAttribute = computeAttributeFromFaceData(
                triangulatedCoordIndex,
                triangulatedFaceColors
              );
            } else {
        ...
        if (normal) {
          if (normalPerVertex === true) {
            ...
            if (normalIndex && normalIndex.length > 0) {
              ...
              var triangulatedNormalIndex = triangulateFaceIndex(
                normalIndex,
                ccw
              );
              ...
              normalAttribute = toNonIndexedAttribute(
                triangulatedCoordIndex,
            ...
            if (normalIndex && normalIndex.length > 0) {
              ...
              var triangulatedFaceNormals = triangulateFaceData(
                flattenFaceNormals,
                coordIndex
              );
              normalAttribute = computeAttributeFromFaceData(
                triangulatedCoordIndex,
                triangulatedFaceNormals
              );
            } else {
          ...
          normalAttribute = computeNormalAttribute(
            triangulatedCoordIndex,
            coord,
            creaseAngle
          );
        ...
        if (texCoord) {
          ...
          if (texCoordIndex && texCoordIndex.length > 0) {
            ...
            var triangulatedTexCoordIndex = triangulateFaceIndex(
              texCoordIndex,
              ccw
            );
            uvAttribute = computeAttributeFromIndexedData(
              triangulatedCoordIndex,
              triangulatedTexCoordIndex,
              texCoord,
              2
            );
          } else {
            ...
            uvAttribute = toNonIndexedAttribute(
              triangulatedCoordIndex,
              new Float32BufferAttribute(texCoord, 2)
            );
          }
        }

        var geometry = new BufferGeometry();
        positionAttribute = toNonIndexedAttribute(
          triangulatedCoordIndex,
          new Float32BufferAttribute(coord, 3)
        );
      ...
      function buildIndexedLineSetNode(node) {
        ...
        if (color) {
          if (colorPerVertex === true) {
            ...
            if (colorIndex.length > 0) {
              ...
              var expandedLineColors = expandLineData(
                flattenLineColors,
                coordIndex
              ); // compute colors for each line segment (rendering primitve)
              colorAttribute = computeAttributeFromLineData(
                expandedLineIndex,
                expandedLineColors
              ); // compute data on vertex level
            } else {
        ...
        var positionAttribute = toNonIndexedAttribute(
          expandedLineIndex,
          new Float32BufferAttribute(coord, 3)
        );
        geometry.setAttribute("position", positionAttribute);
      ...
      function buildElevationGridNode(node) {
        ...
        var uvAttribute = toNonIndexedAttribute(
          indices,
      ...
      function buildExtrusionNode(node) {
        ...
        for (
          var i = 0, j = 0, o = 0, il = spine.length;
          i < il;
          i += 3, j += 2, o += 4
        ) {
        ...
        var normalAttribute = computeNormalAttribute(
          indices,
          vertices,
          creaseAngle
        );
      ...
      function computeAttributeFromIndexedData(
        coordIndex,
        index,
        data,
        itemSize
      ...
      function computeNormalAttribute(index, coord, creaseAngle) {
        ...
        for (var i = 0, l = faces.length; i < l; i++) {
          ...
          var nA = weightedNormal(
            vertexNormals[face.a],
            face.normal,
            creaseAngle
          );
          var nB = weightedNormal(
            vertexNormals[face.b],
            face.normal,
            creaseAngle
          );
          var nC = weightedNormal(
      ...
      function paintFaces(geometry, radius, angles, colors, topDown) {
        ...
        var colorAttribute = new BufferAttribute(
          ...
          for (var j = 1; j < thresholds.length; j++) {
            ...
            if (topDown === true) {
              ...
              if (position.y <= thresholdA.y && position.y > thresholdB.y) {
                t =
                  Math.abs(thresholdA.y - position.y) /
                  Math.abs(thresholdA.y - thresholdB.y);
