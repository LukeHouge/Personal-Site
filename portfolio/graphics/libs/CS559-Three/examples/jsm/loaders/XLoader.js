    ...
    createClass(XAnimationObj, [
      {
        ...
        value: function make(XAnimationInfoArray) {
          ...
          this.length =
            this.hierarchy[0].keys[this.hierarchy[0].keys.length - 1].time;
        },
      },
      {
        key: "clone",
          ...
          for (var i = 0; i < XAnimationInfo.keyFrames.length; i++) {
            ...
            if (XAnimationInfo.keyFrames[i].matrix) {
              ...
              if (this.putFlags.putPos) {
                keyframe.pos = new Vector3().setFromMatrixPosition(
                  keyframe.matrix
                );
              }

              if (this.putFlags.putRot) {
                keyframe.rot = new Quaternion().setFromRotationMatrix(
                  keyframe.matrix
                );
              }

              if (this.putFlags.putScl) {
                keyframe.scl = new Vector3().setFromMatrixScale(
                  keyframe.matrix
                );
              }
            }
    ...
    createClass(XLoader, [
        ...
        value: function _hierarchieParse(_parent, _end) {
          ...
          while (true) {
            ...
            if (find1 > 0 && findEnd > find1) {
              ...
              var nameData = this._readLine(
                this._data.substr(endRead, find1 - endRead - 1)
              ).trim();
              ...
              if (word.length > 0) {
                ...
                if (word.length >= 2) {
                  ...
                  _currentObject.name =
                    word[0] + this.Hierarchies.children.length;
                }
              } else {
              ...
              if (_currentObject.type === "Animation") {
                _currentObject.data = this._data
                  .substr(findNext, findEnd - findNext)
                  .trim();
                var refs = this._hierarchieParse(_currentObject, findEnd + 1);
                endRead = refs.end;
                _currentObject.children = refs.parent.children;
              } else {
                var DataEnder = this._data.lastIndexOf(
                  ";",
                  findNext > 0 ? Math.min(findNext, findEnd) : findEnd
                );
                _currentObject.data = this._data
                  .substr(find1, DataEnder - find1)
                  .trim();
      ...
      },
      {
        ...
        value: function _mainloop() {
          ...
          this._mainProc();
          if (
            this._currentObject.parent ||
            this._currentObject.children.length > 0 ||
            !this._currentObject.worked
          ) {
            setTimeout(function () {
              _this2._mainloop();
            }, 1);
          ...
          while (true) {
            if (!this._currentObject.worked) {
              switch (this._currentObject.type) {
                ...
                case "Mesh":
                  ...
                  this._currentGeo.parentName = this._getParentName(
                    this._currentObject
                  ).trim();
                ...
                case "Animation":
                  if (this._currentAnimeFrames) {
                    this._currentAnime.AnimeFrames.push(
                      this._currentAnimeFrames
                    );
                  }

                  this._currentAnimeFrames = new XAnimationInfo();
                  this._currentAnimeFrames.boneName =
                    this._currentObject.data.trim();
                  break;
                case "AnimationKey":
            ...
            if (this._currentObject.children.length > 0) {
              ...
              if (this._currentObject.worked) {
                if (
                  this._currentObject.parent &&
                  !this._currentObject.parent.parent
                ) {
                  this._changeRoot();
                }
              }
              ...
              switch (this._currentObject.type) {
                case "Mesh":
                  switch (mode) {
                    case 0:
                      this._readVertex1(
                        this._currentObject.data.substr(endRead, find - endRead)
                      );
                      break;
                    case 1:
                      this._readFace1(
      ...
      },
      {
        ...
        value: function _readInt1(start) {
          ...
          return {
            refI: parseInt(
              this._currentObject.data.substr(start, find - start)
            ),
            endRead: find + 1,
          };
        },
      },
      {
        ...
        value: function _readVertex1(line) {
          var data = this._readLine(line.trim())
            ...
            .split(";");
          this._currentGeo.GeometryData.vertices.push(
            parseFloat(data[0]),
            parseFloat(data[1]),
            parseFloat(data[2])
          );
      ...
      },
      {
        ...
        value: function _readNormalVector1(line) {
          var data = this._readLine(line.trim())
            .substr(0, line.length - 2)
            .split(";");
          this._currentGeo.GeometryData.normals.push(
            parseFloat(data[0]),
            parseFloat(data[1]),
            parseFloat(data[2])
          );
      ...
      },
      {
        ...
        value: function _buildGeometry() {
          ...
          for (var i = 0, l = data.indices.length; i < l; i++) {
            ...
            position.push(
              data.vertices[stride3],
              data.vertices[stride3 + 1],
              data.vertices[stride3 + 2]
            );
            normals.push(
              data.normals[stride3],
              data.normals[stride3 + 1],
              data.normals[stride3 + 2]
            );
            skinIndices.push(
            ...
            switch (mode) {
              case 0:
                if (mode_local === 0) {
                  ...
                  var line = this._currentObject.data.substr(
                    endRead,
                    find - endRead
                  );
                  var data = this._readLine(line.trim()).split(";");
                  if (this.IsUvYReverse) {
                    this._currentGeo.GeometryData.uvs.push(
                      parseFloat(data[0]),
                      1 - parseFloat(data[1])
                    );
                  } else {
          ...
          while (true) {
            if (mode_local < 2) {
              ...
              for (var i = 0; i < data.length; i++) {
                this._currentGeo.GeometryData.materialIndices[i] = parseInt(
                  data[i]
                );
              }
        ...
        value: function _setMaterial() {
          ...
          while (true) {
            if (this._currentObject.children.length > 0) {
              ...
              var fileName = localObject.data.substr(
                1,
                localObject.data.length - 2
              );
        ...
        value: function _makePutBoneList(_RootName, _bones) {
          ...
          for (var frame in this.HieStack) {
            if (this.HieStack[frame].name === _RootName || putting) {
              ...
              b.matrixWorld = b.matrix;
              b.FrameTransformMatrix =
                this.HieStack[frame].FrameTransformMatrix;
              b.pos = new Vector3()
                .setFromMatrixPosition(b.FrameTransformMatrix)
                .toArray();
              b.rotq = new Quaternion()
              ...
              b.scl = new Vector3()
                ...
                .toArray();
              if (
                this.HieStack[frame].parentName &&
                this.HieStack[frame].parentName.length > 0
              ) {
      ...
      },
      {
        ...
        value: function _makeOutputGeometry() {
          ...
          if (this._currentGeo.BoneInfs.length > 0) {
            var putBones = [];
            this._makePutBoneList(
              this._currentGeo.baseFrame.parentName,
              putBones
            );
            for (var bi = 0; bi < this._currentGeo.BoneInfs.length; bi++) {
              ...
              for (var bb = 0; bb < putBones.length; bb++) {
                if (
                  putBones[bb].name === this._currentGeo.BoneInfs[bi].boneName
                ) {
                  boneIndex = bb;
                  putBones[bb].OffsetMatrix = new Matrix4();
                  putBones[bb].OffsetMatrix.copy(
                    this._currentGeo.BoneInfs[bi].OffsetMatrix
                  );
                  break;
                }
              }
              ...
              for (
                var vi = 0;
                vi < this._currentGeo.BoneInfs[bi].Indeces.length;
                vi++
              ) {
                ...
                switch (this._currentGeo.VertexSetedBoneCount[nowVertexID]) {
                  case 0:
                    this._currentGeo.GeometryData.skinIndices[stride] =
                      boneIndex;
                    this._currentGeo.GeometryData.skinWeights[stride] = nowVal;
                    break;
                  case 1:
                    this._currentGeo.GeometryData.skinIndices[stride + 1] =
                      boneIndex;
                    this._currentGeo.GeometryData.skinWeights[stride + 1] =
                      nowVal;
                    break;
                  case 2:
                    this._currentGeo.GeometryData.skinIndices[stride + 2] =
                      boneIndex;
                    this._currentGeo.GeometryData.skinWeights[stride + 2] =
                      nowVal;
                    break;
                  case 3:
                    this._currentGeo.GeometryData.skinIndices[stride + 3] =
                      boneIndex;
                    this._currentGeo.GeometryData.skinWeights[stride + 3] =
                      nowVal;
                    break;
                }
            ...
            mesh = new SkinnedMesh(
              bufferGeometry,
              this._currentGeo.Materials.length === 1
                ? this._currentGeo.Materials[0]
                : this._currentGeo.Materials
            );
        ...
        value: function _readAnimationKey() {
          ...
          for (var i = 0; i < data.length; i++) {
            ...
            if (nowKeyType != 4) {
              ...
              for (
                var mm = 0;
                mm < this._currentAnimeFrames.keyFrames.length;
                mm++
              ) {
                if (
                  this._currentAnimeFrames.keyFrames[mm].Frame === keyInfo.Frame
                ) {
        ...
        value: function assignAnimation(_model, _animation) {
          ...
          for (var b = 0; b < model.skeleton.bones.length; b++) {
            ...
            for (var i = 0; i < animation.hierarchy.length; i++) {
              if (
                model.skeleton.bones[b].name === animation.hierarchy[i].name
              ) {
                findAnimation = true;
                var c_key = animation.hierarchy[i].copy();
                c_key.parent = -1;
                if (
                  model.skeleton.bones[b].parent &&
                  model.skeleton.bones[b].parent.type === "Bone"
                ) {
                  for (var bb = 0; bb < put.hierarchy.length; bb++) {
                    if (
                      put.hierarchy[bb].name ===
                      model.skeleton.bones[b].parent.name
                    ) {
                      c_key.parent = bb;
          ...
          model.geometry.animations.push(
            AnimationClip.parseAnimation(put, model.skeleton.bones)
          );
