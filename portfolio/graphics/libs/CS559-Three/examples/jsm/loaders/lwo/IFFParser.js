...
IFFParser.prototype = {
  ...
  setupForm(type, length) {
    ...
    if (!this.currentForm[type]) {
      ...
      console.warn(
        "LWOLoader: form already exists on parent: ",
        type,
        this.currentForm
      );
  ...
  parseLayer(length) {
    ...
    this.currentLayer.parent =
      parsedLength < length ? this.reader.getUint16() : -1; // omitted or -1 for no parent
  },
  ...
  parseUnknownCHUNK(blockID, length) {
    console.warn(
      "LWOLoader: unknown chunk type: " + blockID + " length: " + length
    );
