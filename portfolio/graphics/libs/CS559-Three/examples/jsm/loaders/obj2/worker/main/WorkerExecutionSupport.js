...
WorkerExecutionSupport.prototype = {
  ...
  setTerminateWorkerOnLoad: function (terminateWorkerOnLoad) {
    ...
    if (
      ...
      this.isWorkerLoaded(this.worker.jsmWorker) &&
  ...
  buildWorker: function (codeBuilderInstructions) {
    ...
    if (
      codeBuilderInstructions.isSupportsJsmWorker() &&
      codeBuilderInstructions.isPreferJsmWorker()
    ) {
  ...
  _buildWorkerJsm: function (codeBuilderInstructions) {
    ...
    const workerAvailable = this._buildWorkerCheckPreconditions(
      true,
      timeLabel
    );
    if (!workerAvailable) {
      try {
        const worker = new Worker(codeBuilderInstructions.jsmWorkerUrl.href, {
          type: "module",
        });
        this._configureWorkerCommunication(
          worker,
          true,
          codeBuilderInstructions.defaultGeometryType,
          timeLabel
        );
      } catch (e) {
        jsmSuccess = false;
  ...
  _buildWorkerStandard: function (codeBuilderInstructions) {
    ...
    if (!workerAvailable) {
      ...
      const blob = new Blob([concatenateCode], {
