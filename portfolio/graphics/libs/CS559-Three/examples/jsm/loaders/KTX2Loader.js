  ...
  parse(buffer, onLoad, onError) {
    ...
    this.basisModulePending.then(function () {
      var BasisLzEtc1sImageTranscoder =
        scope.basisModule.BasisLzEtc1sImageTranscoder;
      ...
      var transcoder =
        ktx.getTexFormat() === TextureFormat.UASTC4x4
          ? new UastcImageTranscoder()
          : new BasisLzEtc1sImageTranscoder();

      ktx
        .initMipmaps(transcoder, scope.transcoderConfig)
        .then(function () {
          var texture = new CompressedTexture(
            ktx.mipmaps,
            ktx.getWidth(),
          ...
          texture.minFilter =
            ktx.mipmaps.length === 1 ? LinearFilter : LinearMipmapLinearFilter;
...
class KTX2Container {
  constructor(basisModule, arrayBuffer) {
    ...
    var headerReader = new KTX2BufferReader(
      this.arrayBuffer,
      idByteLength,
      headerByteLength,
      littleEndian
    ...
    var levelReader = new KTX2BufferReader(
      this.arrayBuffer,
      idByteLength + headerByteLength,
      levelByteLength,
      littleEndian
    );
    ...
    this.dfd.numSamples =
      (this.dfd.descriptorBlockSize / 4 - sampleStart) / sampleWords;
    ...
    if (
      this.header.vkFormat !== 0x00 /* VK_FORMAT_UNDEFINED */ &&
      !(
      ...
      throw new Error(
        "THREE.KTX2Loader: Only Basis Universal supercompression is currently supported."
      );
    }
    ...
    var selectorsByteOffset =
      endpointsByteOffset + this.sgd.endpointsByteLength;
    ...
    this.sgd.endpointsData = new Uint8Array(
      this.arrayBuffer,
      endpointsByteOffset,
      this.sgd.endpointsByteLength
    );
    this.sgd.selectorsData = new Uint8Array(
      this.arrayBuffer,
      selectorsByteOffset,
      this.sgd.selectorsByteLength
    );
    this.sgd.tablesData = new Uint8Array(
      this.arrayBuffer,
      tablesByteOffset,
      this.sgd.tablesByteLength
    );
    this.sgd.extendedData = new Uint8Array(
      this.arrayBuffer,
      extendedByteOffset,
      this.sgd.extendedByteLength
    );
  }
  ...
  async initMipmaps(transcoder, config) {
    ...
    var pvrtcTranscodable =
      MathUtils.isPowerOfTwo(width) && MathUtils.isPowerOfTwo(height);
    ...
    if (config.astcSupported) {
      ...
      targetFormat = hasAlpha
        ? TranscodeTarget.BC3_RGBA
        : TranscodeTarget.BC1_RGB;
      this.transcodedFormat = hasAlpha
        ? RGBA_S3TC_DXT5_Format
        : RGB_S3TC_DXT1_Format;
      ...
      targetFormat = hasAlpha
        ? TranscodeTarget.ETC2_RGBA
        : TranscodeTarget.ETC1_RGB /* subset of ETC2 */;
    ...
    if (!this.basisModule.isFormatSupported(targetFormat, texFormat)) {
      throw new Error(
        "THREE.KTX2Loader: Selected texture format not supported by current transcoder build."
      );
    }
    ...
    for (var level = 0; level < this.header.levelCount; level++) {
      ...
      var levelUncompressedByteLength =
      ...
      for (var imageIndex = 0; imageIndex < numImagesInLevel; imageIndex++) {
        ...
        if (texFormat === TextureFormat.UASTC4x4) {
          ...
          encodedData = new Uint8Array(
            this.arrayBuffer,
            this.levels[level].byteOffset + imageOffsetInLevel,
            levelByteLength
          );

          if (this.header.supercompressionScheme === 2 /* ZSTD */) {
            encodedData = this.zstd.decode(
              encodedData,
          ...
          encodedData = new Uint8Array(
            this.arrayBuffer,
            this.levels[level].byteOffset + imageDesc.rgbSliceByteOffset,
  ...
  getTexFormat() {
    ...
    return this.dfd.colorModel === DFDModel.UASTC
      ? TextureFormat.UASTC4x4
      : TextureFormat.ETC1S;
  }
  ...
  getAlpha() {
    ...
    if (
      this.dfd.numSamples === 2 &&
      (this.dfd.samples[1].channelID & 0xf) === DFDChannel.ETC1S.AAA
    ) {
      return true;
    }
