  ...
  parse: function (buffer, path) {
    ...
    Virtulous.KeyFrame = function (time, matrix) {
      ...
      this.lerp = function (nextKey, time) {
        ...
        Virtulous.KeyFrame.tempAniQuat.set(
          keyrot.x,
          keyrot.y,
          keyrot.z,
          keyrot.w
    ...
    Virtulous.KeyFrameTrack = function () {
        ...
        for (var j = 0; j < this.length * this.fps; j++) {
          for (var i = 0; i < this.keys.length; i++) {
            if (this.keys[i].time == j) {
              ...
              this.keys[i].time < j / this.fps &&
              this.keys[i + 1] &&
              this.keys[i + 1].time >= j / this.fps
            ) {
              this._accelTable[j] = i;
              break;
            }
      ...
      this.parseFromThree = function (data) {
        ...
        for (var i = 0; i < track.length; i++) {
          this.addKey(
            new Virtulous.KeyFrame(
              i / fps || track[i].time,
              track[i].targets[0].data
            )
          );
        }
      ...
      this.parseFromCollada = function (data) {
        ...
        for (var i = 0; i < track.length; i++) {
          this.addKey(
            new Virtulous.KeyFrame(i / fps || track[i].time, track[i].matrix)
          );
        }
      ...
      this.setTime = function (time) {
        ...
        for (var i = this.keySearchAccel(time); i < this.keys.length; i++) {
          if (this.keys[i].time == time) {
            ...
            this.keys[i].time < time &&
            this.keys[i + 1] &&
            this.keys[i + 1].time > time
          ) {
            key0 = this.keys[i];
            key1 = this.keys[i + 1];
            break;
    ...
    function aiMesh() {
      ...
      this.hookupSkeletons = function (scene) {
        ...
        for (var i = 0; i < this.mBones.length; i++) {
          ...
          if (bone) {
            ...
            var threeSkeletonRoot = skeletonRoot.toTHREE(scene);
            var threeSkeletonRootBone = cloneTreeToBones(
              threeSkeletonRoot,
              scene
            );
            this.threeNode.add(threeSkeletonRootBone);
      ...
      this.toTHREE = function (scene) {
        ...
        geometry.setIndex(
          new BufferAttribute(new Uint32Array(this.mIndexArray), 1)
        );
        geometry.setAttribute(
          "position",
          new BufferAttribute(this.mVertexBuffer, 3)
        );
        if (this.mNormalBuffer && this.mNormalBuffer.length > 0)
          geometry.setAttribute(
            "normal",
            new BufferAttribute(this.mNormalBuffer, 3)
          );
        if (this.mColorBuffer && this.mColorBuffer.length > 0)
          geometry.setAttribute(
            "color",
            new BufferAttribute(this.mColorBuffer, 4)
          );
        if (this.mTexCoordsBuffers[0] && this.mTexCoordsBuffers[0].length > 0)
          geometry.setAttribute(
            "uv",
            new BufferAttribute(new Float32Array(this.mTexCoordsBuffers[0]), 2)
          );
        if (this.mTexCoordsBuffers[1] && this.mTexCoordsBuffers[1].length > 0)
          geometry.setAttribute(
            "uv1",
            new BufferAttribute(new Float32Array(this.mTexCoordsBuffers[1]), 2)
          );
        if (this.mTangentBuffer && this.mTangentBuffer.length > 0)
          geometry.setAttribute(
            "tangents",
            new BufferAttribute(this.mTangentBuffer, 3)
          );
        if (this.mBitangentBuffer && this.mBitangentBuffer.length > 0)
          geometry.setAttribute(
            "bitangents",
            new BufferAttribute(this.mBitangentBuffer, 3)
          );
        if (this.mBones.length > 0) {
          ...
          geometry.setAttribute(
            "skinWeight",
            new BufferAttribute(new Float32Array(_weights), BONESPERVERT)
          );
          geometry.setAttribute(
            "skinIndex",
            new BufferAttribute(new Float32Array(_bones), BONESPERVERT)
          );
        }
      ...
      this.toTHREE = function () {
        ...
        for (var i = 0; i < this.mProperties.length; i++) {
          if (nameTypeMapping[this.mProperties[i].mKey] == "float")
            mat[namePropMapping[this.mProperties[i].mKey]] =
              this.mProperties[i].dataAsFloat();
          if (nameTypeMapping[this.mProperties[i].mKey] == "color")
            mat[namePropMapping[this.mProperties[i].mKey]] =
              this.mProperties[i].dataAsColor();
          if (nameTypeMapping[this.mProperties[i].mKey] == "bool")
            mat[namePropMapping[this.mProperties[i].mKey]] =
              this.mProperties[i].dataAsBool();
          if (nameTypeMapping[this.mProperties[i].mKey] == "string")
            mat[namePropMapping[this.mProperties[i].mKey]] =
              this.mProperties[i].dataAsString();
    ...
    function ReadBinaryMesh(stream, mesh) {
      ...
      if (c & ASSBIN_MESH_HAS_POSITIONS) {
        if (shortened) {
          ...
          mesh.mVertices = [];
          mesh.mVertexBuffer = stream.subArray32(
            stream.readOffset,
            stream.readOffset + mesh.mNumVertices * 3 * 4
          );
      ...
      if (c & ASSBIN_MESH_HAS_NORMALS) {
        if (shortened) {
          ...
          mesh.mNormals = [];
          mesh.mNormalBuffer = stream.subArray32(
            stream.readOffset,
            stream.readOffset + mesh.mNumVertices * 3 * 4
          );
          stream.Seek(mesh.mNumVertices * 3 * 4, aiOrigin_CUR);
      ...
      if (c & ASSBIN_MESH_HAS_TANGENTS_AND_BITANGENTS) {
        if (shortened) {
          ...
          mesh.mTangents = [];
          mesh.mTangentBuffer = stream.subArray32(
            stream.readOffset,
            stream.readOffset + mesh.mNumVertices * 3 * 4
          );
          stream.Seek(mesh.mNumVertices * 3 * 4, aiOrigin_CUR);
          mesh.mBitangents = [];
          mesh.mBitangentBuffer = stream.subArray32(
            stream.readOffset,
            stream.readOffset + mesh.mNumVertices * 3 * 4
          );
          stream.Seek(mesh.mNumVertices * 3 * 4, aiOrigin_CUR);
      ...
      for (var n = 0; n < AI_MAX_NUMBER_OF_COLOR_SETS; ++n) {
        ...
        if (shortened) {
          ...
          mesh.mColors[n] = [];
          mesh.mColorBuffer = stream.subArray32(
            stream.readOffset,
            stream.readOffset + mesh.mNumVertices * 4 * 4
          );
      ...
      if (shortened) {
        ...
        for (var i = 0; i < mesh.mNumFaces; ++i) {
          ...
          if (f.mNumIndices === 3) {
            ...
            throw new Error(
              "Sorry, can't currently triangulate polys. Use the triangulate preprocessor in Assimp."
            );
          }
        }
      }
